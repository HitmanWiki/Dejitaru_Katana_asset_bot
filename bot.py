...
import json
import traceback
from utils import serialize_table, get_back_keyboard, getHTML
from backend import get_user_transactions, get_user_balances, set_user_address, get_user_address

# P_TIMEZONE = pytz.timezone(config.TIMEZONE)
# TIMEZONE_COMMON_NAME = config.TIMEZONE_COMMON_NAME

bot = telebot.TeleBot('6399773656:AAFLEIjEqcJIsDLExzmuMdE-JE8L0NP-x7g')
print(bot)
...
def start_command(message):
    ...
    keyboard.row(
        telebot.types.InlineKeyboardButton(
            'Powered by Katana', callback_data='a'
        )
    )
    bot.send_message(
        message.chat.id,
...
def help_command(message):
   ...
   keyboard.add(
       telebot.types.InlineKeyboardButton(
           'Message the developer 💻', url='telegram.me/hyperbolic1'
       )
   )
   bot.send_message(
       message.chat.id,
